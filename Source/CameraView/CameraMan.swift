...
class CameraMan {
  ...
  //let queue = DispatchQueue(label: "no.hyper.ImagePicker.Camera.SessionQueue", attributes: dispatch_queue_attr_make_with_qos_class(DispatchQueueAttributes.serial, DispatchQueueAttributes.qosBackground, 0))
  

  let queue = DispatchQueue(label:"no.hyper.ImagePicker.Camera.SessionQueue", attributes:.qosBackground)
